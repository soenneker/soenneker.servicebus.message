...
public sealed class ServiceBusMessageUtil : IServiceBusMessageUtil
{
    ...
    private ServiceBusMessage? BuildMessageStjUtf8<TMessage>(TMessage message, string type) where TMessage : Messages.Base.Message
    {
        ...
        {
            LogCriticalError(ex, type, message, false);
            return null;
        }
    }
    ...
    private ServiceBusMessage? BuildMessageNewtonsoftString<TMessage>(TMessage message, string type) where TMessage : Messages.Base.Message
    {
        try
        {
            ...
            string serialized = JsonConvert.SerializeObject(message, _newtonsoftSettings);
        ...
        {
            LogCriticalError(ex, type, message, true);
            return null;
        }
    }
    ...
    private void LogCriticalError(Exception ex, string type, object message, bool useNewtonsoft)
    {
        ...
        if (_log)
        {
            try
            {
                string? serialized = useNewtonsoft
                    ? JsonConvert.SerializeObject(message, _newtonsoftSettings)
